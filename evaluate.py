...
def evaluate(args, model, step):    
    # Get dataset
    data_loader = prepare_dataloader(args.data_path, "val.txt", batch_size=16, shuffle=False) 
    ...
    mel_l = sum(mel_l_list) / (1 if len(mel_l_list) == 0 else len(mel_l_list))
    d_l = sum(d_l_list) / (1 if len(d_l_list) == 0 else len(d_l_list))
    f_l = sum(f_l_list) / (1 if len(f_l_list) == 0 else len(f_l_list))
    e_l = sum(e_l_list) / (1 if len(e_l_list) == 0 else len(e_l_list))
